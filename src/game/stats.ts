...
export type Attribute = "str" | "dex" | "int" | "vit";

export const ATTRIBUTES: Attribute[] = ["str", "dex", "int", "vit"];
...
/** Every derived/aggregate stat the game understands. */
export type StatKey =
  | "maxHealth"
  | "maxMana"
  | "healthRegen" // per second
  | "manaRegen" // per second
  | "moveSpeed" // world units per second
  | "physicalDamage"
  | "fireDamage"
  | "coldDamage"
  | "lightningDamage"
  | "attackSpeed" // multiplier on skill use rate
  | "critChance" // 0..1
  | "critMultiplier" // e.g. 1.5 = 150%
  | "armor" // flat physical mitigation
  | "damageReduction" // 0..1 fractional
  | "minionDamage" // increased % applied to minions
  | "minionLife" // increased % applied to minion life
  | "areaOfEffect" // increased % radius
  | "projectileCount"; // additional projectiles (flat)
...
export interface StatModifier {
  ...
  kind: ModKind;
  value: number;
  /** Human-readable source tag, e.g. "Level", "Iron Circlet", "Craft". */
  source?: string;
}

/** A bag of primary attribute points. */
 ...
 *   VIT -> life + life regen
 */
export function attributeModifiers(attr: AttributeBlock): StatModifier[] {
...
export function computeStats(
  ...
  const moreFactors: Partial<Record<StatKey, number>> = {};

  for (const m of mods) {
    if (m.kind === "flat") flat[m.stat] = (flat[m.stat] ?? 0) + m.value;
    else if (m.kind === "increased") increased[m.stat] = (increased[m.stat] ?? 0) + m.value;
    else moreFactors[m.stat] = (moreFactors[m.stat] ?? 1) * (1 + m.value);
  }
