...
import { StatKey, StatModifier, ModKind } from "../stats.js";
import { EquipSlot } from "./bases.js";

export type AffixType = "prefix" | "suffix";

export interface AffixTier {
  tier: number; // 1 = best
  /** The exact value granted at this tier. */
  value: number;
  /** Minimum item level required to use this tier in crafting. */
  itemLevel: number;
}

export interface AffixDef {
  id: string;
  /** Display name fragment, e.g. "of the Bear" or "Flaming". */
  name: string;
  type: AffixType;
  stat: StatKey;
  kind: ModKind;
  ...
  /** Tiers ordered from best (tier 1) to worst. */
  tiers: AffixTier[];
  /** Group tag: an item may only carry one affix per group (e.g. one "life"). */
  group: string;
}

const ALL_ARMOUR: EquipSlot[] = ["helmet", "chest", "gloves", "boots", "shield"];
...
function tiers(entries: [number, number, number][]): AffixTier[] {
...
export function affixesForSlot(slot: EquipSlot, type?: AffixType): AffixDef[] {
  return AFFIXES.filter(
    (a) => a.allowedSlots.includes(slot) && (type === undefined || a.type === type)
  );
}

export function tierOf(affix: AffixDef, tier: number): AffixTier {
  const t = affix.tiers.find((x) => x.tier === tier);
  if (!t) throw new Error(`Affix ${affix.id} has no tier ${tier}`);
  return t;
}

/** Best (lowest-number) tier available at a given item level. */
export function bestTierForItemLevel(affix: AffixDef, itemLevel: number): AffixTier | null {
