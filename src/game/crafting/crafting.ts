...
import { Item, RolledAffix, MAX_PREFIXES, MAX_SUFFIXES } from "../items/item.js";
import { getBase, ItemBase } from "../items/bases.js";
import {
  AffixDef,
  getAffix,
  affixesForSlot,
  tierOf,
  AffixType,
} from "../items/affixes.js";
import { MaterialCost, MaterialId } from "./materials.js";
...
export interface CraftResult {
  ...
  reason?: string;
  cost: MaterialCost;
  /** A PREVIEW of the resulting item (not yet committed). */
  preview?: Item;
}
...
export function affixTierCost(affix: AffixDef, tier: number): MaterialCost {
  ...
  if (t.itemLevel >= 45) cost.rune = (cost.rune ?? 0) + 1;
  return cost;
}

export function fabricateCost(base: ItemBase): MaterialCost {
  // Higher item-level bases cost more shards and some essence.
  const shards = 3 + Math.floor(base.itemLevel / 5);
  const cost: MaterialCost = { shard: shards };
  if (base.itemLevel >= 20) cost.essence = 2;
  if (base.itemLevel >= 40) cost.rune = 1;
  return cost;
}

export function removeCost(): MaterialCost {
  return { catalyst: 1 };
}
...
function countByType(item: Item, type: AffixType): number {
  return item.affixes.filter((a) => getAffix(a.affixId).type === type).length;
}

function canHostAffix(item: Item, affix: AffixDef): { ok: boolean; reason?: string } {
...
function tierAllowedByItemLevel(item: Item, affix: AffixDef, tier: number): boolean {
  const t = tierOf(affix, tier);
...
export function addAffix(
  ...
  if (!host.ok) return { ok: false, reason: host.reason, cost };
...
export function addableAffixes(item: Item): AffixDef[] {
...
export function permittedTiers(item: Item, affix: AffixDef): number[] {
  return affix.tiers
