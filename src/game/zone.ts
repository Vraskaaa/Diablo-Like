...
import { Vec2, distance, vec2 } from "../engine/math.js";
...
export const CRYPT_ZONE: ZoneDef = {
...
export class ZoneRunner {
  ...
  private interWaveDelay = 3;
  private delayTimer = 0;
  /** Callback fired when a new wave starts (for UI banners). */
  onWaveStart?: (wave: WaveDef, index: number, total: number) => void;
  onZoneCleared?: () => void;

  constructor(world: GameWorld, def: ZoneDef) {
    this.world = world;
    this.def = def;
  }
  ...
  update(dt: number): void {
    ...
    if (this.waveActive) {
      ...
      const remaining = this.world.entities.filter(
        (e) => e instanceof Enemy && !e.dead
      ).length;
      if (remaining === 0) {
        this.waveActive = false;
        if (this.currentWave >= this.def.waves.length - 1) {
          this.cleared = true;
          if (this.onZoneCleared) this.onZoneCleared();
          return;
        }
    ...
    for (const group of wave.groups) {
      for (let i = 0; i < group.count; i++) {
        ...
        if (group.kind === "caster") {
          enemy.rangedAttack = (from, to, damage) =>
            this.world.makeEnemyRangedAttack(from, to, damage);
        }
  ...
  private pickSpawnPoint(): Vec2 {
    ...
    for (let attempt = 0; attempt < 60; attempt++) {
      ...
      if (!player || distance(pos, player.pos) > 260) return pos;
    }
    // Fallback: a corner.
