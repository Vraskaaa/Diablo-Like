...
import { Vec2, normalize, add, scale, distance, angleTo, vec2 } from "../engine/math.js";
import { StatKey } from "./stats.js";
import { DamageType, buildDamage } from "./damage.js";
import { Projectile } from "./entities/projectile.js";
import { Combatant } from "./entities/combatant.js";
import { isHostile } from "./entities/projectile.js";

/** Minimal view of the caster a skill needs. */
export interface Caster extends Combatant {
  stats: Record<StatKey, number>;
  /** Summoner-only: track owned minion count for cap enforcement. */
  minionCount?: number;
  minionCap?: number;
...
export interface SkillContext {
  caster: Caster;
  /** World-space aim point (mouse position translated to world). */
  target: Vec2;
  world: World;
  /** Visual/impact sink for AoE hits, particles, etc. */
  fx: SkillFx;
}

/** Effects the game layer provides for skills to spawn visuals / AoE hits. */
export interface SkillFx {
  spawnProjectile: (p: Projectile) => void;
  /** Deal an AoE damage burst centered at a point; returns hit count. */
  areaBurst: (
    center: Vec2,
    radius: number,
  ...
  /** Spawn a transient visual ring/flash. */
  spawnFx: (kind: string, at: Vec2, radius: number, color: string) => void;
  /** Melee arc hit in front of the caster. */
  meleeArc: (
    caster: Caster,
    angle: number,
    arc: number,
...
export type SkillTargeting = "point" | "direction" | "self";
...
export interface Skill {
  ...
  description: string;
  manaCost: number;
  /** Cooldown in seconds (0 = only limited by attack speed). */
  cooldown: number;
  /** Base cast time in seconds, scaled by attackSpeed for attack skills. */
  castTime: number;
  targeting: SkillTargeting;
  icon: string; // single glyph used for the skill button
  isAttack: boolean; // attacks scale cast rate with attackSpeed
  cast: (ctx: SkillContext) => void;
}

// -------- helpers --------

function aimDir(caster: Caster, target: Vec2): Vec2 {
  return normalize({ x: target.x - caster.pos.x, y: target.y - caster.pos.y });
}

/** Spread N projectiles evenly across a total angle (radians). */
function spreadAngles(count: number, spread: number, baseAngle: number): number[] {
  if (count <= 1) return [baseAngle];
  const out: number[] = [];
  const start = baseAngle - spread / 2;
  const stepA = spread / (count - 1);
  for (let i = 0; i < count; i++) out.push(start + stepA * i);
  return out;
}
...
const raise_skeleton: Skill = {
  ...
  cast: ({ caster, target, world }) => {
    if (caster.requestSummon) caster.requestSummon("skeleton", target, world);
  },
};

const summon_golem: Skill = {
  id: "summon_golem",
  name: "Summon Golem",
  ...
  isAttack: false,
  cast: ({ caster, target, world }) => {
    if (caster.requestSummon) caster.requestSummon("golem", target, world);
  },
};

const bone_spear: Skill = {
  id: "bone_spear",
  name: "Bone Spear",
...
const command: Skill = {
  ...
  isAttack: false,
  cast: ({ caster, target, world }) => {
    if (caster.commandMinions) caster.commandMinions(target, world);
  },
};
...
// ============================================================
// DoT status support (used by poison arrow). Stored on the target.
// ============================================================
...
const dotRegistry = new WeakMap<Combatant, DotState>();

function applyDot(target: Combatant, dps: number, duration: number): void {
  dotRegistry.set(target, { dps, remaining: duration });
}

/** Called each tick by the game loop for all combatants. */
export function tickDots(target: Combatant, dt: number, world: World): void {
  const dot = dotRegistry.get(target);
  if (!dot) return;
  dot.remaining -= dt;
  const damage = dot.dps * dt;
  target.receiveDamage(
    { physical: 0, fire: 0, cold: damage, lightning: 0, isCrit: false },
    world
  );
  if (dot.remaining <= 0) dotRegistry.delete(target);
}
