...
export enum TileType {
  Floor = 0,
  Wall = 1,
  Rubble = 2, // walkable decorative
  Water = 3, // blocks movement
}
...
export interface TileMap {
  ...
  spawn: Vec2; // world-space suggested spawn point
}
...
export function isBlocked(map: TileMap, worldX: number, worldY: number): boolean {
 ...
 */
export function generateArena(width: number, height: number, seed: number): TileMap {
  ...
  for (let i = 0; i < clusterCount; i++) {
    ...
    if (Math.abs(px - cx) < 3 && Math.abs(py - cy) < 3) continue; // keep spawn clear
  ...
  for (let i = 0; i < tiles.length; i++) {
    if (tiles[i] === TileType.Floor && rng.chance(0.06)) tiles[i] = TileType.Rubble;
  }
