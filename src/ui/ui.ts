...
import { PLAYABLE_CLASSES, CLASSES, ClassId } from "../game/classes.js";
import { Character } from "../game/character.js";
import { Item } from "../game/items/item.js";
import { RARITY_COLORS } from "../game/items/item.js";
import { EQUIP_SLOTS, SLOT_NAMES, EquipSlot, ITEM_BASES, basesForSlot } from "../game/items/bases.js";
import { ATTRIBUTES, ATTRIBUTE_NAMES, STAT_NAMES, StatKey, ALL_STATS } from "../game/stats.js";
...
export interface UiCallbacks {
  onSelectClass: (classId: ClassId, name: string) => void;
  onEquip: (item: Item) => void;
  onUnequip: (slot: EquipSlot) => void;
  onAllocateAttribute: (attr: (typeof ATTRIBUTES)[number]) => void;
  onCraftChanged: () => void; // recompute stats after a craft
  getCharacter: () => Character | null;
  onSkillClick: (index: number) => void;
  onRestart: () => void;
  /** Resume a saved character, if one exists. */
  onContinue?: () => void;
  /** Delete the current save. */
  onDeleteSave?: () => void;
  /** Summary of the existing save for the class-select screen, or null. */
  savedSummary?: () => { name: string; className: string; level: number } | null;
}

export class UI {
  private root: HTMLElement;
  private cb: UiCallbacks;
  private craftingSelectedUid: number | null = null;
  ...
  showClassSelect(): void {
    ...
    const contRow = screen.querySelector("#continue-row") as HTMLElement;
    if (saved) {
      const box = el("div");
      box.style.cssText =
    ...
    for (const id of PLAYABLE_CLASSES) {
      ...
      card.addEventListener("click", () => {
        selected = id;
        grid.querySelectorAll(".class-card").forEach((c) =>
          (c as HTMLElement).style.removeProperty("border-color")
        );
        card.style.borderColor = cls.color;
        (screen.querySelector("#begin-btn") as HTMLButtonElement).disabled = false;
      });
      grid.appendChild(card);
    }

    const nameInput = screen.querySelector("#hero-name") as HTMLInputElement;
    const beginBtn = screen.querySelector("#begin-btn") as HTMLButtonElement;
  ...
  updateHud(data: {
    ...
    data.cooldowns.forEach((cd, i) => {
      const cdEl = this.hudEl!.querySelector(`#cd-${i}`) as HTMLElement | null;
      if (!cdEl) return;
      if (cd > 0.05) {
    ...
    slots.forEach((s, i) =>
      (s as HTMLElement).classList.toggle("active", i === data.activeSkill)
    );
  }
  ...
  private openModal(title: string, bodyBuilder: (body: HTMLElement) => void): void {
    this.closeModal();
  ...
  showCharacter(): void {
    ...
    this.openModal("Character", (body) => {
      ...
      for (const attr of ATTRIBUTES) {
        ...
        plus.textContent = "+";
        if (c.unspentAttributePoints <= 0) plus.style.opacity = "0.3";
  ...
  private attachTooltip(cell: HTMLElement, item: Item): void {
  ...
  showCrafting(): void {
    ...
    this.openModal("Crafting Bench — Deterministic (no RNG)", (body) => {
      ...
      const baseSelect = el("select") as HTMLSelectElement;
      ...
      const fabBtn = el("button", "craft-btn") as HTMLButtonElement;
      ...
      for (const item of craftable) {
        ...
        if (item.uid === this.craftingSelectedUid) row.style.borderColor = "var(--gold-bright)";
  ...
  private buildCraftActions(right: HTMLElement, c: Character, item: Item | null): void {
    ...
    // Equip button.
    const equipBtn = el("button", "craft-btn") as HTMLButtonElement;
    ...
    for (const affix of options) {
      ...
      const row = el("div", "craft-row");
      const tierSelect = el("select") as HTMLSelectElement;
      ...
      const addBtn = el("button", "craft-btn") as HTMLButtonElement;
      addBtn.textContent = "Add";
      const updateCost = () => {
        const t = Number(tierSelect.value);
        const cost = affixTierCost(affix, t);
        (info.querySelector(`#cost-${affix.id}`) as HTMLElement).textContent = `— ${formatCost(cost)}`;
      };
      ...
      addBtn.addEventListener("click", () => {
        ...
        if (!res.ok) this.flash(res.reason ?? "Cannot add");
        else {
          this.cb.onCraftChanged();
          this.showCrafting();
        }
    ...
    if (item.affixes.length > 0) {
      ...
      for (const roll of [...item.affixes]) {
        ...
        const upBtn = el("button", "craft-btn") as HTMLButtonElement;
        ...
        upBtn.addEventListener("click", () => {
          ...
          if (!res.ok) this.flash(res.reason ?? "Cannot upgrade");
          else {
            this.cb.onCraftChanged();
            this.showCrafting();
          }
        ...
        // Remove.
        const rmBtn = el("button", "craft-btn") as HTMLButtonElement;
        rmBtn.textContent = `Remove (${formatCost(removeCost())})`;
        rmBtn.addEventListener("click", () => {
          const res = removeAffix(c, item, roll.affixId, true);
          if (!res.ok) this.flash(res.reason ?? "Cannot remove");
          else {
            this.cb.onCraftChanged();
            this.showCrafting();
          }
  ...
  showCenterMessage(big: string, small: string, death = false): void {
