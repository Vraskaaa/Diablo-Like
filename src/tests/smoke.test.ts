...
console.log("== Headless world simulation ==");
{
  ...
  const player = new Player(
    { classDef: character.classDef, stats, level: 1 },
    { ...map.spawn },
    world
  );
  ...
  world.update(1 / 60);
  assert(
    world.entities.some((e) => e === player),
    "Player is present in the world after the first update tick"
  );
  ...
  for (let i = 0; i < 60 * 8; i++) {
    ...
    const nearest = world.entities.find((e) => e instanceof Enemy && !e.dead) as
      | Enemy
      | undefined;
    if (nearest) {
...
console.log("== Summoner minions ==");
{
  ...
  const stats = character.computeStats();
  const player = new Player(
    { classDef: character.classDef, stats, level: 5 },
    { ...map.spawn },
    world
  );
  player.resources.fill(stats);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    for (let f = 0; f < 40; f++) world.update(1 / 60);
  }
