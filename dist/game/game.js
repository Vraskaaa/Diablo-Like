...
import { generateArena, drawTileMap } from "../engine/tilemap.js";
...
const KEY_TO_SKILL = { q: 0, w: 1, e: 2, r: 3 };
export class Game {
    canvas;
    ctx;
    input;
    ...
    constructor(canvas, uiRoot) {
        ...
        this.loop = new GameLoop({ update: (dt) => this.update(dt), render: () => this.render() }, 60);
    }
    ...
    beginRun(character) {
        ...
        const player = new Player({ classDef: character.classDef, stats, level: character.level }, { ...map.spawn }, world);
    ...
    // ---- input -> player intent ----
    handleInput() {
        if (!this.player || !this.world || this.gameOver)
            return;
        if (this.ui.isModalOpen())
            return; // don't act while a panel is open
        // Toggle panels.
        if (this.input.wasPressed("c"))
            this.ui.showCharacter();
    ...
    castActiveSkill() {
        if (!this.player || !this.world || !this.character)
            return;
    ...
    update(dt) {
        ...
        if (this.world && !this.gameOver) {
            ...
            if (this.player)
                this.camera.follow(this.player.pos, 0.12);
            // Autosave every ~5s.
    ...
    handleEnemyKilled(enemy) {
        ...
        for (const [id, amt] of Object.entries(drop)) {
            if (amt)
                this.character.addMaterial(id, amt);
        }
        if (this.world) {
    ...
    refreshStats(fillOnLevel = false) {
