...
import { getBase } from "./bases.js";
import { getAffix, tierOf } from "./affixes.js";
...
export class Item {
    ...
    uniqueName;
    constructor(baseId, affixes = [], uid) {
    ...
    displayName() {
        ...
        let name = base.name;
        if (pre.length > 0)
            name = `${getAffix(pre[0].affixId).name} ${name}`;
        if (suf.length > 0)
            name = `${name} ${getAffix(suf[0].affixId).name}`;
        return name;
    }
    clone() {
        return new Item(this.baseId, this.affixes.map((a) => ({ ...a })), this.uid);
    }
    toJSON() {
        return {
            uid: this.uid,
...
export function starterItemsFor(classId) {
    switch (classId) {
        case "warrior":
            return [
                new Item("iron_sword", [
