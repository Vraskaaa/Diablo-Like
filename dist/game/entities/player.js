...
import { Minion, MINION_CONFIGS } from "./minion.js";
export class Player extends Combatant {
    ...
    constructor(model, pos, fx) {
        ...
        this.requestSummon = (kind, at, world) => this.summon(kind, at, world);
    ...
    update(dt, world) {
        ...
        for (const [id, t] of this.cooldowns) {
            if (t > 0)
                this.cooldowns.set(id, Math.max(0, t - dt));
        }
        if (this.castLock > 0)
    ...
    tryCast(skill, world) {
        ...
        if (this.getCooldown(skill.id) > 0)
            return false;
        if (!this.resources.canAfford(skill.manaCost))
            return false;
