...
const { saveCharacter, loadCharacter, hasSave, clearSave } = await import("../game/persistence.js");
let failures = 0;
...
console.log("== Persistence round-trip ==");
{
    ...
    assert(loaded.materialCount("shard") === c.materialCount("shard"), `Materials preserved exactly (shard=${loaded.materialCount("shard")})`);
    ...
    assert(loaded.equipment.weapon.affixes.length === 1, "Weapon affix preserved through save");
    assert(loaded.inventory.length === c.inventory.length, "Inventory count preserved");
    assert(Math.abs(loaded.computeStats().attackSpeed - c.computeStats().attackSpeed) < 1e-9, "Computed attackSpeed identical after load");
    clearSave();
