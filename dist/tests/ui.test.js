...
character.materials = { ...startingMaterials() };
...
console.log("== Crafting bench: Fabricate button ==");
{
    ...
    const fabBtn = root.findByText("BUTTON", "Fabricate");
    ...
    assert(character.materialCount("shard") < shardStart, `Materials were spent (shard ${shardStart} -> ${character.materialCount("shard")})`);
}
console.log("== Crafting bench: Add affix button ==");
{
    // Select the freshly-created item and add an affix via its button.
    const item = character.inventory[character.inventory.length - 1];
    ui.craftingSelectedUid = item.uid;
    ui.showCrafting();
    const addBtn = root.findByText("BUTTON", "Add");
